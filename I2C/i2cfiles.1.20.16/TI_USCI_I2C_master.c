...
void TI_USCI_I2C_receiveinit(unsigned char slave_address, 
                             unsigned char prescale){
  P1SEL |= SDA_PIN + SCL_PIN;                 // Assign I2C pins to USCI_B0 **CHANGED FROM P3SEL to P1SEL
...
//------------------------------------------------------------------------------
void TI_USCI_I2C_transmit(unsigned char byteCount, unsigned char *field){ //changed return type to check the transmitted value
  TI_transmit_field = field;
  byteCtr = byteCount;
  UCB0CTL1 |= UCTR + UCTXSTT;                 // I2C TX, start condition
  //return TI_transmit_field; //added for debugging/checking
}
